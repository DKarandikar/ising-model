...
cdef numpy.ndarray[int, ndim=2, mode='c'] mcsetup_cy(int grid_size):
    '''Setups up the initial lattice and spins'''
    cdef int i
    cdef int j
    cdef numpy.ndarray[int, ndim=2, mode='c'] lattice = numpy.zeros((grid_size, grid_size), dtype=int)
...
cdef numpy.ndarray[int, ndim=2, mode='c'] mcmove_cy(numpy.ndarray[int, ndim=2, mode='c'] lattice, int move_n, float exp_low, float exp_high):
    '''Flip a spin if the energy change is beneficial'''
    cdef int pos_x
    cdef int pos_y
    cdef int new_e
...
def mcrun(float temp, int n_0, int n_max, int move_n, int grid_size):
    ...
    cdef numpy.ndarray[int, ndim=2, mode='c'] lattice = mcsetup_cy(grid_size)

    cdef float exponential_low = c_exp(-1*4*(1/temp))
    cdef float exponential_high = c_exp(-1*8*(1/temp))

    cdef int _

    for _ in range(n_0):
        lattice = mcmove_cy(lattice, move_n, exponential_low, exponential_high)
